...
- **player-tagged Authorization**: the tag-mode socket event binds the acting player to the authenticated socket connection (`client.id`) rather than trusting a client-supplied ID, and rejects self-tags — prevents a connected client from impersonating the current IT player; server-side cooldown/freeze (`server/tagAuthorization.js`) mirrors `TagMode.applyTag`'s rules, and `server/itHandoff.js` reassigns the IT player on disconnect instead of leaving it pointing at a disconnected client
